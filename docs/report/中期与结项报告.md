...
- **订单模块**：发布订单（地点、物品、备注）；跑腿员浏览未接订单并接单（一个订单只能一人接）；更新订单进度；未接订单可取消。
...
![骑手认证界面](./assets/page6.png)

- 管理员后台界面

![管理员后台界面](./assets/page7.png)
...
本系统已部署在校园网服务器上，用户无需进行任何安装或环境配置，只需满足以下条件即可使用：
...
2. 填写以下信息：  
    ...
    - 小费(元)（如 5.0） 
3. 点击“发布订单”。  
4. 发布成功后，订单出现在“我的订单”列表中，状态为“待接单”，在被接单之前可选“取消订单”。
5. 骑手接单后状态更新为“已接单”。
...
**更新订单进度**  
1. 跑腿员可在“我的接单”列表中找到已接订单。  
2. 根据实际配送阶段，点击对应按钮更新状态（例如“已取件”“配送中”“已完成”）。  
...
问题与不足：
- 密码目前采用明文存储，存在安全隐患；接口未引入 JWT 身份认证和权限控制
- Git 提交规范问题，开发初期提交信息不够规范

改进方向：
- 引入 Spring Security 和 BCrypt 加密，增加登录令牌机制
- git提交已在中期检查后整改为 feat:、config: 等约定式提交格式
...
可见于：https://github.com/30CampusExpress/campus-express/commits/main/
